...
exports.getBudgetProgress = exports.createBudget = exports.generateFinancialInsights = exports.processNaturalLanguageQuery = exports.processTransaction = void 0;
 ...
 */
exports.processNaturalLanguageQuery = functions.https.onCall(async (data, context) => {
 ...
 */
exports.generateFinancialInsights = functions.pubsub.schedule("every 24 hours").onRun(async (context) => {
    try {
        // Get all users
 ...
 * Get budget progress
 */
exports.getBudgetProgress = functions.https.onCall(async (data, context) => {
    // Ensure user is authenticated
    if (!(context === null || context === void 0 ? void 0 : context.auth)) {
        throw new functions.https.HttpsError("unauthenticated", ERROR_MESSAGES.UNAUTHENTICATED);
    }
    ...
    try {
        ...
        // Verify ownership
        if ((budget === null || budget === void 0 ? void 0 : budget.userId) !== userId) {
        ...
        return {
            success: true,
            budget: Object.assign(Object.assign({}, budget), { spent: totalSpent, remaining: Math.max(budget.amount - totalSpent, 0), progress, isOverBudget: totalSpent > budget.amount }),
        };
    }
    catch (error) {
...
async function storeTransactionVector(transaction) {
...
async function updateUserFinancialSummary(userId, transaction) {
...
async function processQueryWithAI(query, transactions) {
    ...
    if (query.toLowerCase().includes("spend") && query.toLowerCase().includes("food")) {
        const foodTransactions = transactions.filter((t) => t.category.toLowerCase().includes("food") && t.type === "expense");
...
async function generateInsightsWithAI(transactions) {
    // This would integrate with Fetch AI's agents
